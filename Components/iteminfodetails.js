...
const ProductDetail = ({
    ...
    const shop = "690592f03c7028da16b118c6"

    useEffect(() => {
        if (selectedShop?._id && categotyid) {
            getProduct();
        }
    }, [categotyid]);
    ...
    const getProduct = async () => {
        ...
        try {
            ...
            if (res.status && Array.isArray(res.data) && res.data.length > 0) {
                ...
                // toaster({ type: "success", message: "Product fetched successfully" });
            } else {
                toaster({ type: "error", message: "No product found" });
            }
        } catch (err) {
            loader(false);
            setLoading(false);
            toaster({ type: "error", message: "Failed to fetch product" });
        }
    };
    ...
    const handleNext = () => {
        ...
        // You can close modal or navigate here
    };
    ...
    return (
        <div className="fixed inset-0 z-[9999999999] flex justify-center items-center pt-20">
            <div className="bg-black/80 backdrop-blur-sm  overflow-y-auto rounded-2xl shadow-2xl w-full max-w-lg mx-3 relative max-h-[90vw] md:h-[550px] h-[800px]">
